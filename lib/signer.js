...
var Signer = function ()
{
  if ("object" !== typeof config.rsa)
    throw UserError('Config error: config.rsa is not an object');

  if ("string" !== typeof config.rsa.d)
    throw UserError('Config.error: config.rsa.d is not a string');

  if ("string" !== typeof config.rsa.e)
    throw UserError('Config error: config.rsa.e is not a string');
...
Signer.prototype.sign = function (infoData, signreq)
{
  ...
  // Verify 1 < m < n
  if (!m.gt(1))
    throw UserError("Invalid signing request: m <= 1");
  if (!m.lt(this.n))
    throw UserError("Invalid signing request: m >= n");

  // Verify Jacobi ( m | n ) = 1
  if (m.jacobi(this.n) !== 1)
    throw UserError("Invalid signing request: Jacobi symbol ( m | n ) != -1");
...
Signer.prototype.test = function (infoData, signreq)
{
    var verifyPublicInfo = function (info) {
        var match = pubInfoRegex.exec(info);

        if (!match) throw new Error("Invalid public info, unexpected format");

        if (match[1] != match[2].length)
        throw UserError("Invalid public info, servername length mismatch");

        //if (match[2] !== config.serverName)
        //throw UserError("Invalid public info, wrong servername");

        if (match[3] != match[4].length)
        throw UserError("Invalid public info, username length mismatch");

        if (match[5] != match[6].length)
        throw UserError("Invalid public info, purpose length mismatch");
    ...
    var info = verifyPublicInfo(infoData);
    ...
    console.log("verify jacobi, ",arguments, this)
    console.log("m.jacobi(this.n) = ", m.jacobi(this.n))
    // Verify Jacobi ( m | n ) = 1
    if (m.jacobi(this.n) !== 1)
    throw UserError("Invalid signing request: Jacobi symbol ( m | n ) != -1");
